                  ...
                  <div class="col-12">
                    <div class="card">
                      <div class="card-header">
                        <h3 class="card-title float-end">جدول المستخدمين</h3>
                        <button type="button" data-bs-toggle="modal" data-bs-target="#addUser" class="btn btn-info">إضافة عنصر جديد</button>
                      </div>
                      <!-- /.card-header -->
                      <div class="card-body">
                          ...
                          <tbody>
                            @foreach ($users as $user)
                              <tr>
                                ...
                                <td>
                                  <button type="button" data-bs-toggle="modal" data-bs-target="#deleteUser"class="btn-danger btn" title="delete" onclick="deleteUserItem({{$user->id}})"><i class="fas fa-trash"></i></button>
                                
                                  <button class="btn-info btn editUser" title="edit"  type="button" 
                                data-id="{{$user->id}}" 
                                data-name="{{$user->name}}" 
                                data-email="{{$user->email}}"  
                                data-password="{{$user->password}}"  
                                data-active="{{$user->active}}" 
                                  data-bs-toggle="modal" data-bs-target="#editUser"><i class="fas fa-pen"></i></button>
                                </td>
                              </tr>
                            @endforeach
                          </tbody>
...
@section("scripts")
    ...
    <script>
      ...
      function deleteUserItem(id){
        ...
        $('#editUser-form-id').val(id)
        $('#editUser-form-name').val(name)
        $('#editUser-form-email').val(email)
        $('#editUser-form-password').val(password)
        $('#editUser-form-active').val(active)
